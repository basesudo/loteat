...
//            'landing_page_settings_full_url'=> $data['web_app_landing_page_settings_storage']??'public',
    ...
    public function get_zone(Request $request)
    {
        ...
        $zones = Zone::with('modules')->latest()->get(['id', 'status', 'cash_on_delivery', 'digital_payment','agency_payment', 'offline_payment', 'increased_delivery_fee', 'increased_delivery_fee_status', 'increase_delivery_charge_message']);
        if (count($zones) < 1) {
            return response()->json([
                'errors' => [
    ...
    private function getDefaultPaymentMethods()
    {
        ...
        $methods = Setting::where('is_active', 1)->whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago','ezypay','antom','nova2pay','pricpay','uprimer'])->get();
