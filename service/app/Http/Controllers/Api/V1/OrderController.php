...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cash_on_delivery,digital_payment,agency_payment,wallet,offline_payment',
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        if ($request->payment_method == 'agency_payment') {
            $order->agency_id = Str::uuid()->toString();
        }
        ...
        $order->card_info = $request->payment_details ? json_encode($request->payment_details) : null;
