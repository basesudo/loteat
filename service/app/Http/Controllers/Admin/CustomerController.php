...
class CustomerController extends Controller
{
    ...
    public function agencyPayment(User $customer, Request $request)
    {
        $customer->agency_payment = $request->status;
        $customer->save();

        try {
            if ($request->status == 0) {
                // 当禁用代付功能时
                if (isset($customer->cm_firebase_token) && Helpers::getNotificationStatusData('customer','customer_agency_payment_disable','push_notification_status')) {
                    $data = [
                        'title' => translate('messages.agency_payment_disabled'),
                        'description' => translate('messages.your_agency_payment_has_been_disabled'),
                        'order_id' => '',
                        'image' => '',
                        'type' => 'agency_payment_disable'
                    ];
                    Helpers::send_push_notif_to_device($customer->cm_firebase_token, $data);
                ...
                if (config('mail.status') && Helpers::get_mail_status('agency_payment_disable_mail_status_user') == '1' && Helpers::getNotificationStatusData('customer','customer_agency_payment_disable','mail_status')) {
                    Mail::to($customer->email)->send(new \App\Mail\UserStatus('agency_payment_disabled', $customer->f_name.' '.$customer->l_name));
                }
            } else {
                // 当启用代付功能时
                if (Helpers::getNotificationStatusData('customer','customer_agency_payment_enable','push_notification_status') && isset($customer->cm_firebase_token)) {
                    $data = [
                        'title' => translate('messages.agency_payment_enabled'),
                        'description' => translate('messages.your_agency_payment_has_been_enabled'),
                        'order_id' => '',
                        'image' => '',
                        'type' => 'agency_payment_enable'
                    ];
                    Helpers::send_push_notif_to_device($customer->cm_firebase_token, $data);
                ...
                if (config('mail.status') && Helpers::get_mail_status('agency_payment_enable_mail_status_user') == '1' && Helpers::getNotificationStatusData('customer','customer_agency_payment_enable','mail_status')) {
                    Mail::to($customer->email)->send(new \App\Mail\UserStatus('agency_payment_enabled', $customer->f_name.' '.$customer->l_name));
                }
            }
        } catch (\Exception $e) {
        ...
        Toastr::success(translate('messages.agency_payment') . translate('messages.status_updated'));
        return back();
    }

    public function rechargeAgencyPayment(User $customer, Request $request)
    {
        $customer->recharge_agency_payment = $request->status;
        $customer->save();

        try {
            if ($request->status == 0) {
                // 当禁用充值代付功能时
                if (isset($customer->cm_firebase_token) && Helpers::getNotificationStatusData('customer','customer_recharge_agency_payment_disable','push_notification_status')) {
                    $data = [
                        'title' => translate('messages.recharge_agency_payment_disabled'),
                        'description' => translate('messages.your_recharge_agency_payment_has_been_disabled'),
                        'order_id' => '',
                        'image' => '',
                        'type' => 'recharge_agency_payment_disable'
                    ];
                    Helpers::send_push_notif_to_device($customer->cm_firebase_token, $data);
                ...
                if (config('mail.status') && Helpers::get_mail_status('recharge_agency_payment_disable_mail_status_user') == '1' && Helpers::getNotificationStatusData('customer','customer_recharge_agency_payment_disable','mail_status')) {
                    Mail::to($customer->email)->send(new \App\Mail\UserStatus('recharge_agency_payment_disabled', $customer->f_name.' '.$customer->l_name));
                }
            } else {
                // 当启用充值代付功能时
                if (Helpers::getNotificationStatusData('customer','customer_recharge_agency_payment_enable','push_notification_status') && isset($customer->cm_firebase_token)) {
                    $data = [
                        'title' => translate('messages.recharge_agency_payment_enabled'),
                        'description' => translate('messages.your_recharge_agency_payment_has_been_enabled'),
                        'order_id' => '',
                        'image' => '',
                        'type' => 'recharge_agency_payment_enable'
                    ];
                    Helpers::send_push_notif_to_device($customer->cm_firebase_token, $data);
                ...
                if (config('mail.status') && Helpers::get_mail_status('recharge_agency_payment_enable_mail_status_user') == '1' && Helpers::getNotificationStatusData('customer','customer_recharge_agency_payment_enable','mail_status')) {
                    Mail::to($customer->email)->send(new \App\Mail\UserStatus('recharge_agency_payment_enabled', $customer->f_name.' '.$customer->l_name));
                }
            }
        } catch (\Exception $e) {
            Toastr::warning(translate('messages.push_notification_failed'));
        ...
        Toastr::success(translate('messages.recharge_agency_payment') . translate('messages.status_updated'));
        return back();
    }
    ...
    public function view(Request $request,$id)
    {
        ...
        if (isset($customer)) {
            ...
            // 查询用户钱包支付记录
            $wallet_payments = WalletPayment::where('user_id', $id)
                ->whereNotNull('agency_id')  // 确保 agency_id 不为 null
                ->where('agency_id', '!=', '')  // 确保 agency_id 不为空字符串
                ->when(isset($key), function($query) use($key){
            ...
            return view('admin-views.customer.customer-view', compact('customer', 'orders', 'total_order_amount', 'wallet_payments'));
        }
