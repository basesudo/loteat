...
class CustomerWalletController extends Controller
{
     ...
     */
    public function agency_payment_details($agency_id)
    {
        // 查询指定代理商ID的支付信息，按时间倒序排列
        $agency_payments = \App\Models\AgencyAddFundInfo::where('agency_id', $agency_id)
            ->latest() // 按创建时间倒序排序
            ->paginate(config('default_pagination'));
        
        return view('admin-views.customer.wallet.agency-payment-details', compact(
            'agency_payments', 
            'agency_id',
        ));
    }

    public function deduct_fund_view()
    {
        if (BusinessSetting::where('key', 'wallet_status')->first()->value != 1) {
            Toastr::error(trans('messages.customer_wallet_disable_warning_admin'));
            return back();
        }
        return view('admin-views.customer.wallet.deduct_balance');
    }

    public function deduct_fund(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'customer_id'=>'exists:users,id',
            'amount'=>'numeric|min:.01',
        ...
        if (!$user) {
            return response()->json(['errors'=>[
                'message'=>trans('messages.customer_not_found')
            ]], 200);
        }
        ...
        $wallet_transaction = CustomerLogic::create_wallet_transaction($request->customer_id, $deduct_amount, 'deduct_fund_by_admin', $request->referance);

        if($wallet_transaction)
        {
            try{
                // 可以添加扣减余额的推送通知
                Helpers::deduct_fund_push_notification($request->customer_id);
                
                // 可以添加扣减余额的邮件通知
                // if(config('mail.status') && Helpers::get_mail_status('deduct_fund_mail_status_user') == '1' &&  Helpers::getNotificationStatusData('customer','customer_deduct_fund_from_wallet','mail_status') ) {
                //     Mail::to($wallet_transaction->user->email)->send(new \App\Mail\DeductFundFromWallet($wallet_transaction));
                // }
            }catch(\Exception $ex)
            {
                info($ex->getMessage());
