...
// dd( $request['commission_business_model']);
    ...
    public function payment_index()
    {
        ...
        $data_values = Setting::whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago','ezypay','antom','nova2pay','pricpay','uprimer'])->get();
    ...
    public function payment_config_update(Request $request)
    {
        ...
        $validation = [
            'gateway' => 'required|in:ssl_commerz,paypal,stripe,razor_pay,senang_pay,paytabs,paystack,paymob_accept,paytm,flutterwave,liqpay,bkash,mercadopago,ezypay,antom,nova2pay,pricpay,uprimer',
        ...
        } elseif ($request['gateway'] == 'ezypay') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'partner_code' => 'required_if:status,1',
                'credential_code' => 'required_if:status,1'
            ];
        }elseif ($request['gateway'] == 'nova2pay') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'accountId' => 'required_if:status,1',
                'md5Key' => 'required_if:status,1',
                'rsaPublicKey' => 'required_if:status,1',
            ];
        }elseif ($request['gateway'] == 'pricpay') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'merchant_no' => 'required_if:status,1',
                'secret_key' => 'required_if:status,1',
            ];
        } elseif ($request['gateway'] == 'antom') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'merchant_private_key' => 'required_if:status,1',
                'antom_public_key' => 'required_if:status,1',
                'client_id' => 'required_if:status,1'
            ];
        } elseif ($request['gateway'] == 'uprimer') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'secret_key' => 'required_if:status,1',
                'access_code' => 'required_if:status,1',
                'app_id' => 'required_if:status,1'
            ];
    ...
    // 代付分享页面
    public function agency_payment_share()
    {
        $agency_payment_share = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'agency_payment_share')->first();
        $agency_payment_share_status = DataSetting::where('type', 'admin_landing_page')->where('key', 'agency_payment_share_status')->first();
        return view('admin-views.business-settings.agency_payment_share', compact('agency_payment_share', 'agency_payment_share_status'));
    }

    public function agency_payment_share_update(Request $request)
    {
        $this->update_data($request, 'agency_payment_share');
        Toastr::success(translate('messages.agency_payment_share_updated'));
        return back();
    }

    public function agency_payment_share_status($status)
    {
        $this->policy_status_update('agency_payment_share_status', $status);
        return response()->json(['status' => "changed"]);
    }

    // 代付感谢页面
    public function agency_payment_thank_you()
    {
        $agency_payment_thank_you = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'agency_payment_thank_you')->first();
        $agency_payment_thank_you_status = DataSetting::where('type', 'admin_landing_page')->where('key', 'agency_payment_thank_you_status')->first();
        return view('admin-views.business-settings.agency_payment_thank_you', compact('agency_payment_thank_you', 'agency_payment_thank_you_status'));
    }

    public function agency_payment_thank_you_update(Request $request)
    {
        $this->update_data($request, 'agency_payment_thank_you');
        Toastr::success(translate('messages.agency_payment_thank_you_updated'));
        return back();
    }

    public function agency_payment_thank_you_status($status)
    {
        $this->policy_status_update('agency_payment_thank_you_status', $status);
        return response()->json(['status' => "changed"]);
    }

    // 代付充值页面
    public function agency_payment_add_fund()
    {
        $agency_payment_add_fund = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'agency_payment_add_fund')->first();
        $agency_payment_add_fund_status = DataSetting::where('type', 'admin_landing_page')->where('key', 'agency_payment_add_fund_status')->first();
        return view('admin-views.business-settings.agency_payment_add_fund', compact('agency_payment_add_fund', 'agency_payment_add_fund_status'));
    }

    public function agency_payment_add_fund_update(Request $request)
    {
        $this->update_data($request, 'agency_payment_add_fund');
        Toastr::success(translate('messages.agency_payment_add_fund_updated'));
        return back();
    }

    public function agency_payment_add_fund_status($status)
    {
        $this->policy_status_update('agency_payment_add_fund_status', $status);
        return response()->json(['status' => "changed"]);
    }

    public function agency_payment_success()
    {
        $agency_payment_success = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'agency_payment_success')->first();
        $agency_payment_success_status = DataSetting::where('type', 'admin_landing_page')->where('key', 'agency_payment_success_status')->first();
        return view('admin-views.business-settings.agency_payment_success', compact('agency_payment_success', 'agency_payment_success_status'));
    }

    public function agency_payment_success_update(Request $request)
    {
        $this->update_data($request, 'agency_payment_success');
        Toastr::success(translate('messages.agency_payment_success_updated'));
        return back();
    }

    public function agency_payment_success_status($status)
    {
        $this->policy_status_update('agency_payment_success_status', $status);
        return response()->json(['status' => "changed"]);
    }
