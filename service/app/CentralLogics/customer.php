...
class CustomerLogic
{
    ...
    public static function create_wallet_transaction($user_id, float $amount, $transaction_type, $referance)
    {
        ...
        } else if (in_array($transaction_type, ['order_place', 'partial_payment'])) {
            $debit = $amount;
        } else if ($transaction_type == 'deduct_fund_by_admin') {
            // 处理管理员扣减余额的情况
            if ($amount < 0) {
                $debit = abs($amount); // 负数转为正数记录到debit
            } else {
                $debit = $amount; // 如果传入正数，直接记录到debit
            }
            $credit = 0.0;
        }
        ...
        try {
            ...
            DB::commit();
            if (in_array($transaction_type, ['loyalty_point', 'order_place', 'add_fund_by_admin', 'referrer','partial_payment', 'deduct_fund_by_admin'])) return $wallet_transaction;
            return true;
        } catch (\Exception $ex) {
            info($ex->getMessage());
            DB::rollback();
