...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function deduct_fund_push_notification($user_id){
        $customer_push_notification_status=self::getNotificationStatusData('customer','customer_deduct_fund_from_wallet','push_notification_status' );

        $user= User::where('id',$user_id)->first();
        if ($customer_push_notification_status && $user?->cm_firebase_token) {
            $data = [
                'title' => translate('messages.fund_deducted'),
                'description' => translate('messages.fund_deducted_from_your_wallet'),
                'order_id' => '',
                'image' => '',
                'type' => 'deduct_fund',
                'order_status' =>'',
