...
@php($zone_instruction = session()?->get('zone-instruction') ?? '0')
    <div class="content container-fluid">
        ...
        data-original-title="{{translate('messages.draw_your_zone_on_the_map')}}">{{translate('messages.draw_your_zone_on_the_map')}}</span></label>
            ...
            @php($agency_payment=\App\CentralLogics\Helpers::get_business_settings('agency_payment_status'))
            <div class="col-12">
                <div class="card">
                        ...
                        <table id="columnSearchDatatable"
                               ...
                               <thead class="thead-light">
                                <tr>
                                    ...
                                    @endif
                                    @if ($agency_payment && $agency_payment['status']==1)
                            ...
                            @php($non_mod = (count($zone->modules)>0 && $non_mod == 0) ? $non_mod:$non_mod+1 )
                                <tr>
                                    ...
                                    @if ($agency_payment && $agency_payment['status']==1)
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm" for="agency_paymentCheckbox{{$zone->id}}">
                                            <input type="checkbox" data-id="agency_payment-{{$zone['id']}}" data-title="{{ $zone->agency_payment?translate('Want_to_disable_’Agency_Payment’?'):translate('Want_to_enable_’Agency_Payment’?') }}" data-message="{{ $zone->agency_payment? translate('If_yes,_the_agency_payment_option_will_be_hidden_during_checkout.'):translate('If_yes,_Customers_can_choose_the_’Agency_Payment’_option_during_checkout.')}}" class="toggle-switch-input status_form_alert" id="agency_paymentCheckbox{{$zone->id}}" {{$zone->agency_payment?'checked':''}}>
                                            <span class="toggle-switch-label">
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                                        <form action="{{route('admin.business-settings.zone.agency-payment',[$zone['id'],$zone->agency_payment?0:1])}}" method="get" id="agency_payment-{{$zone['id']}}">
                                        </form>
                                    </td>
                                    @endif
                                    @if ($config && $config['status']==1)
