...
这样 `/public/assets/...` 会被正确映射到 `/var/www/html/service/public/assets/...`。

**注意**：项目结构调整后，Laravel 代码位于 `service/` 文件夹中，所以 Nginx 配置中的路径需要添加 `/service` 前缀。
...
- 导致 `/storage/app/public/...` 的请求被错误映射到 `/var/www/html/service/public/storage/...`
- 而实际图片文件在 `/var/www/html/service/storage/app/public/...`

**注意**：项目结构调整后，Laravel 代码位于 `service/` 文件夹中。
...
```bash
docker-compose exec app rm -rf /var/www/html/service/public/storage
docker-compose exec app php artisan storage:link
```

**注意**：项目结构调整后，路径已变为 `/var/www/html/service/public/storage`。
...
location ^~ /storage/app/public/ {
    alias /var/www/html/service/storage/app/public/;
    expires 1M;
    add_header Cache-Control "public, immutable";
}
...
# 处理 storage 路径（软链接）
location ^~ /storage/ {
    alias /var/www/html/service/public/storage/;
    expires 1M;
    add_header Cache-Control "public, immutable";
}
```

**注意**：项目结构调整后，所有路径都需要添加 `/service` 前缀。
...
# 设置目录权限
chown -R www-data:www-data /var/www/html/service/storage
chown -R www-data:www-data /var/www/html/service/bootstrap/cache
chmod -R 755 /var/www/html/service/storage
chmod -R 755 /var/www/html/service/bootstrap/cache
```

或在 Dockerfile 中设置：

```dockerfile
RUN chown -R www-data:www-data /var/www/html/service \
    && chmod -R 755 /var/www/html/service/storage \
    && chmod -R 755 /var/www/html/service/bootstrap/cache
```
...
```bash
# 进入 PHP 容器
docker-compose exec app ls -la /var/www/html/service/public/storage

# 应该显示类似：
# lrwxrwxrwx 1 root root 25 Apr 14 10:15 /var/www/html/service/public/storage -> /var/www/html/service/storage/app/public
```

**注意**：项目结构调整后，路径已变为 `/var/www/html/service/...`。
