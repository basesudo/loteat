...
import agency_payment from "../checkout/assets/agency_payment.png";
...
const WalletBoxComponent = (props) => {
  ...
  const formSubmitHandler = (values) => {
    ...
    // 针对 Agency Payment 的特殊处理
    if (values.payment_method === "agency_payment") {
      const payloadData = {
        ...values,
        payment_method: "agency_payment",
      };
      
      mutate(payloadData, {
        onSuccess: async (response) => {
          setLoading(false);
          if (response?.agency_id && response?.status === "success") {
            // 设置代理支付详情
            setAgencyPaymentDetails({
            ...
            // 处理其他响应情况
            toast.error(t("Failed to process agency payment request"));
          }
        },
        onError: (error) => {
          setLoading(false);
  ...
  // 处理关闭代理支付模态框
  const handleAgencyPaymentModalClose = () => {
    setAgencyPaymentModalOpen(false);
    // 清理状态
    setAgencyPaymentDetails(null);
  };

  // 处理导航到代理支付页面
  React.useEffect(() => {
    if (readyToNavigate && agencyPaymentDetails?.agency_id) {
      // 关闭所有模态框
      setAgencyPaymentModalOpen(false);
      setOpen(false);
      // 跳转到代理支付页面
      Router.push(`/agency-add-fund?agency_id=${agencyPaymentDetails.agency_id}`);
      // 重置状态
      setReadyToNavigate(false);
    }
  }, [readyToNavigate, agencyPaymentDetails]);
              ...
              {formik.values.amount > 0 && (
                <>
                  <Stack>
                    ...
                    {/* 添加 Agency Payment 选项 */}
                    {configData?.add_fund_payment_status === true && 
                    (profileInfo?.recharge_agency_payment === 1 || profileInfo?.recharge_agency_payment === 1) && (
                      <CustomRadioBox key="agency_payment">
                        <label
                          className={value === "agency_payment" ? "active" : ""}
                        >
                          <input
                            type="radio"
                            name="payment_method"
                            value="agency_payment"
                            onChange={(e) => {
                              setValue(e.target.value);
                              formik.handleChange(e);
                            }}
          ...
          >
            ...
            <CustomStackFullWidth spacing={1.5}>
              <Stack direction="row" justifyContent="space-between">
                <Typography fontWeight="500">{t("Amount")}</Typography>
                <Typography>{getAmountWithSign(agencyPaymentDetails.amount)}</Typography>
              </Stack>

              <Stack direction="row" justifyContent="space-between">
                <Typography fontWeight="500">{t("Payment Link")}</Typography>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <Typography fontWeight="500">{t("Share Link")}</Typography>
                  <Tooltip title={t("Copy Link")}>
                    <IconButton
                      size="small"
                      onClick={() => {
                        copyToClipboard(
                          `${window.location.origin}/agency-add-fund?agency_id=${agencyPaymentDetails.agency_id}`
                        ).then(success => {
                          if (success) {
