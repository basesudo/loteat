...
import useGetRegionCurrency  from "../../../api-manage/hooks/react-query/region-currency/useGetRegionCurrency";
...
const BasicInformationForm = ({
  ...
  const { f_name, l_name, phone, email, image, currency, region } = data;
  ...
  // 获取地区和币种数据
  const { data: regionCurrencyData, isLoading: isRegionCurrencyLoading } = useGetRegionCurrency();

  // 从API数据中提取地区和币种选项
  const regionOptions = regionCurrencyData?.data?.regions?.map(region => ({
    value: region.region_code,
    label: region.region_name
  })) || [];

  const currencyOptions = regionCurrencyData?.data?.currencies?.map(currency => ({
  ...
  const handleReset = () => {
    ...
    profileFormik.setFieldValue("currency", "USD");
    profileFormik.setFieldValue("region", "CN");
          ...
          <Grid item md={6} xs={12}>
            <FormControl sx={{ width: "100%" }}>
              ...
              <Select
                ...
                label={t("Currency")}
                onChange={profileFormik.handleChange}
                disabled={isRegionCurrencyLoading}
                sx={{
                  height: "45px",
                }}
              >
          ...
          <Grid item md={6} xs={12}>
            <FormControl sx={{ width: "100%" }}>
              ...
              <Select
                ...
                id="region-select"
                name="region"
                value={profileFormik.values.region}
                label={t("Region")}
                onChange={profileFormik.handleChange}
                disabled={isRegionCurrencyLoading}
                sx={{
